...
namespace tModloaderDiscordBot.Modules
{
	...
	public class PermissionModule : ConfigModuleBase
	{
		...
		public async Task Default(string command)
		{
			...
			foreach (ulong permission in Config.Permissions.GetPermissionsForKey(command))
		...
		public async Task SetAsync(string command, params ulong[] ids)
		{
			...
			IUserMessage msg = await ReplyAsync("Updating permissions...");
		...
		public class AdminModule : ConfigModuleBase
		{
			...
			public async Task ResetAsync([Remainder] string rem = "")
			{
				...
				{
					await ReplyAsync(
						"You are about to reset all permissions, are you sure you want to continue? Issue the command again within 1 minute to affirm.");
					resetFlag = true;
				...
				{
					(int pC, int aC, int bC) = Config.Permissions.ResetPermissions();
					await Config.Update();
					resetFlag = false;
					await ReplyAsync(
						$"All permission were reset. Removed {aC} admins, {pC} permissions and {bC} blocks.");
				}
			}

			[Command("assign")]
			[Alias("add", "-a")]
			public async Task AssignAsync(IRole role) => await AssignAsync(role.Id);

			[Command("assign")]
			[Alias("add", "-a")]
			public async Task AssignAsync(IGuildUser user) => await AssignAsync(user.Id);
			...
			[Command("unassign")]
			[Alias("delete", "-d", "-ua")]
			public async Task UnassignAsync(IRole role) => await UnassignAsync(role.Id);

			[Command("unassign")]
			[Alias("delete", "-d", "-ua")]
			public async Task UnassignAsync(IGuildUser user) => await UnassignAsync(user.Id);
