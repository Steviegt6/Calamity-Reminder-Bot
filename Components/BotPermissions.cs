...
namespace tModloaderDiscordBot.Components
{
	...
	{
		[JsonProperty] private readonly IList<ulong> _admins = new List<ulong>();

		[JsonProperty] private readonly IList<ulong> _blocked = new List<ulong>();
		...
		public bool HasPermissionsForKey(string key) => _permissions.ContainsKey(key.ToLowerInvariant());

		public void MakeAdmin(ulong userId) => _admins.Add(userId);

		public void RemoveAdmin(ulong userId) => _admins.Remove(userId);

		public bool IsBlocked(ulong userId) => _blocked.Contains(userId);

		public bool IsAdmin(ulong userId) => _admins.Contains(userId);
