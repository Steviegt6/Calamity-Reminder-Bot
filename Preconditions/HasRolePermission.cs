	...
	internal class HasPermissionAttribute : PreconditionAttribute
	{
			...
			CommandInfo command, IServiceProvider services)
		{
			bool CheckIfBlocked(IGuildUser user, BotPermissions gPerms) =>
				gPerms.IsBlocked(context.User.Id)
			...
			CommandService commandService = (CommandService) services.GetService(typeof(CommandService));
			...
			// Get the permissions
			PermissionService permissionService = (PermissionService) services.GetService(typeof(PermissionService));
			...
			BotPermissions permissions = permissionService.GetGuildPermissions();
			...
			string cmd = await BotUtils.SearchCommand(commandService, context, command.Module.Name);
			...
			if (!permissions.HasPermissionsForKey(cmd))
			{
				return PreconditionResult.FromError(
					$"No permissions setup for the command `{cmd}` but required to use it.");
			}
