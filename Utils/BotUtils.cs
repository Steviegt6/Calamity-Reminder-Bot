...
namespace tModloaderDiscordBot.Utils
{
	public static class BotUtils
	{
		...
		public static byte[] UnicodeGetBytes(this string content) => Encoding.Unicode.GetBytes(content);

		public static string UnicodeGetString(this byte[] buffer) => Encoding.Unicode.GetString(buffer);
		...
		public static long GetCurrentUnixTimestampMillis() => (long) GetCurrentUnixTimespan().TotalMilliseconds;

		public static DateTime DateTimeFromUnixTimestampMillis(long millis) => UnixEpoch.AddMilliseconds(millis);

		public static long GetCurrentUnixTimestampSeconds() => (long) GetCurrentUnixTimespan().TotalSeconds;

		public static TimeSpan GetCurrentUnixTimespan() => DateTime.UtcNow - UnixEpoch;
		...
		{
			T[] enumerable = ids as T[] ?? ids.ToArray();
		...
		public static string SurroundWith(this string text, string surrounder) => $"{surrounder}{text}{surrounder}";
		...
		public static ConcurrentDictionary<TKey, TSource> ToConcurrentDictionary<TSource, TKey>(
			...
			ToConcurrentDictionary(source, keySelector, IdentityFunction<TSource>.Instance, null);

		public static ConcurrentDictionary<TKey, TSource> ToConcurrentDictionary<TSource, TKey>(
			this IEnumerable<TSource> source, Func<TSource, TKey> keySelector, IEqualityComparer<TKey> comparer) =>
			ToConcurrentDictionary(source, keySelector, IdentityFunction<TSource>.Instance, comparer);

		public static ConcurrentDictionary<TKey, TElement> ToConcurrentDictionary<TSource, TKey, TElement>(
			this IEnumerable<TSource> source, Func<TSource, TKey> keySelector,
			Func<TSource, TElement> elementSelector) =>
			ToConcurrentDictionary(source, keySelector, elementSelector, null);
		...
		public static async Task<string> SearchCommand(CommandService commandService, ICommandContext commandContext,
			string command)
		{
			...
			string output = "";
			bool found = FindDeepCommand(commandService.Modules, command, ref output);
		...
		public static bool FindDeepCommand(IEnumerable<ModuleInfo> modules, string command, ref string output)
		{
			...
			{
				string o = $"{output}{mod.Name} ";

				if (module && (mod.Name.EqualsIgnoreCase(text) || mod.Aliases.Any(x => x.EqualsIgnoreCase(text)))
				    || mod.Commands.Any(x => x.Name.EqualsIgnoreCase(text) ||
				                             x.Aliases.Any(y => y.EqualsIgnoreCase(text))))
				{
					output = module
						? $"module:{o}"
						: $"{o}{text}";
